...
func (*Install) Usage() string {
	return `install [flags] <name> [args...]  -- if provided, args are passed as arguments to the runtime
`
}
...
func writeConfig(c map[string]interface{}, path string) error {
	...
	if err := ioutil.WriteFile(path+"~", old, 0644); err != nil {
		return fmt.Errorf("error backing up config file %q: %v", path, err)
	}
