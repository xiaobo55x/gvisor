...
func TestDistribution(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			map[string][]float64{
				// Injected packets on dev0 get distributed among endpoints bound to dev0 or unbound.
				"dev0": []float64{0.333, 0.333, 0, 0, 0, 0.333},
				// Injected packets on dev1 get distributed among endpoints bound to dev1 or unbound.
				"dev1": []float64{0, 0, 0.25, 0.25, 0.25, 0.25},
			...
			for device, wantedDistribution := range test.wantedDistributions {
				t.Run(device, func(t *testing.T) {
					...
					if got, want := len(test.endpoints), len(wantedDistribution); got != want {
						t.Fatalf("len(test.endpoints) got %d, want %d", got, want)
					}
