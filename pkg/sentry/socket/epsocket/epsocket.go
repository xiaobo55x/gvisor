...
func toLinuxTCPInfo(i tcp.InfoOption) linux.TCPInfo {
	...
	return linux.TCPInfo{
		...
		ATO:    0,
		SndMss: uint32(i.Sender.MaxPayloadSize),
		...
		LastDataSent: uint32(i.Sender.LastSendTime.Nanoseconds() / int64(time.Millisecond)),
		LastAckSent:  0, // Not tracked by Linux.
		LastDataRecv: uint32(i.RcvLastData.Nanoseconds() / int64(time.Millisecond)),
		LastAckRecv:  uint32(i.RcvLastAck.Nanoseconds() / int64(time.Millisecond)),
