...
import (
	...
	fs2 "gvisor.dev/gvisor/pkg/sentry/fs"
	"gvisor.dev/gvisor/pkg/sentry/fs/ext/disklayout"
	"gvisor.dev/gvisor/pkg/sentry/memmap"
	linux2 "gvisor.dev/gvisor/pkg/sentry/syscalls/linux"
...
func newDirectroy(inode inode, newDirent bool) (*directory, error) {
	...
	if inode.diskInode.Flags().Index {
		// TODO(b/134676337): Support hash tree directories. Until then '.' and '..'
		//                    should be read in.
...
func (fd *directoryFD) IterDirents(ctx context.Context, cb vfs.IterDirentsCallback) error {
	...
	for ; child != nil; child = child.Next() {
		...
		if child.diskDirent != nil {
			...
			if !ok {
				// We will need to read the inode off disk. Do not increment ref count here
				// because this inode is not being added to the dentry tree.
				fs.mu.Lock()
				childInode, err := fs.getOrCreateInodeLocked(child.diskDirent.Inode())
				fs.mu.Unlock()
				if err != nil {
					// Usage of the file description after the error is undefined. This
					// implementation would continue reading from the next dirent.
					fd.off++
					dir.childList.InsertAfter(child, fd.iter)
					return err
				}
				childType = fs2.ToInodeType(childInode.diskInode.Mode().FileType())
			}

			if !cb.Handle(vfs.Dirent{
				Name: child.diskDirent.FileName(),
				Type: linux2.ToDirentType(childType),
